...
def menu_dns_lookup():
    ...
    try:
        ...
        print(f"  {GRY}Reverse DNS :{R} {WHT}{hostname}{R}")
    except:
        print(f"  {GRY}Reverse DNS :{R} {RED}Tidak ditemukan{R}")

    # SSL info sebagai tambahan
    try:
        ctx = ssl.create_default_context()
        ...
        print(f"  {GRY}SSL Issuer  :{R} {WHT}{issuer.get('organizationName', 'N/A')}{R}")
        print(f"  {GRY}SSL Expire  :{R} {WHT}{cert.get('notAfter', 'N/A')}{R}")
    except:
        pass
...
def menu_whois():
    banner()
    print(f"  {CYN}{BLD}[ WHOIS LOOKUP ]{R}\n")
    print(f"  {GRY}Cari info pemilik & registrasi sebuah domain.{R}\n")
    domain = input(f"  {WHT}Masukkan domain{R} {GRY}(contoh: google.com){R}\n  > ").strip()
    if not domain:
        print(f"\n  {RED}Domain tidak boleh kosong!{R}")
        input(f"\n  {GRY}Tekan ENTER untuk kembali...{R}")
    ...
    domain = domain.replace("https://","").replace("http://","").strip("/")
    print(f"\n  {GRY}Mengambil data WHOIS untuk {domain}...{R}\n")

    try:
        url = f"https://api.whois.vu/?q={domain}&json"
        req = urllib.request.Request(url, headers={"User-Agent": "Halim-Soc/1.0"})
        with urllib.request.urlopen(req, timeout=10) as resp:
            raw = resp.read().decode(errors="ignore")
        data = json.loads(raw)

        print(f"  {BLD}{WHT}{'─'*50}{R}")
        print(f"  {CYN}{BLD}  HASIL WHOIS: {domain}{R}")
        print(f"  {BLD}{WHT}{'─'*50}{R}\n")

        whois_text = data.get("whois", raw)
        # Filter & tampilkan baris penting
        keywords = ["registrar","registered","expir","updated","created","name server",
                    "status","registrant","country","dnssec","organization"]
        lines_shown = 0
        for line in whois_text.splitlines():
            ...
            if any(k in low for k in keywords) and ":" in line:
                ...
                if len(parts) == 2 and parts[1].strip():
                    print(f"  {GRY}{parts[0].strip():<20}:{R} {WHT}{parts[1].strip()}{R}")
                    lines_shown += 1
            if lines_shown >= 20:
                break

        if lines_shown == 0:
            print(f"  {YLW}Data WHOIS tidak bisa diparse. Coba cek manual di:{R}")
            print(f"  {CYN}https://whois.domaintools.com/{domain}{R}")

    except Exception as e:
        print(f"  {RED}Gagal mengambil data WHOIS: {e}{R}")
        print(f"\n  {YLW}Coba cek manual di:{R}")
        print(f"  {CYN}https://whois.domaintools.com/{domain}{R}")
...
def menu_subdomain():
    banner()
    print(f"  {CYN}{BLD}[ SUBDOMAIN FINDER ]{R}\n")
    print(f"  {GRY}Cari subdomain yang aktif dari sebuah domain.{R}\n")
    domain = input(f"  {WHT}Masukkan domain{R} {GRY}(contoh: google.com){R}\n  > ").strip()
    if not domain:
        print(f"\n  {RED}Domain tidak boleh kosong!{R}")
        input(f"\n  {GRY}Tekan ENTER untuk kembali...{R}")
    ...
    SUBDOMAINS = [
        ...
        "api","dev","staging","test","beta","demo","app","mobile","m",
        "blog","shop","store","cdn","static","assets","img","images",
        "vpn","remote","secure","login","auth","sso","support","help",
        "docs","wiki","forum","community","news","media","video","stream",
        "db","database","mysql","postgres","redis","mongo","elastic",
        "jenkins","gitlab","github","jira","confluence","grafana","kibana",
        "cpanel","whm","plesk","webdisk","autodiscover","autoconfig",
    ]

    print(f"\n  {GRY}Scanning {len(SUBDOMAINS)} subdomain untuk {domain}...{R}\n")
    ...
    with ThreadPoolExecutor(max_workers=30) as ex:
        ...
        for future in as_completed(futures):
            ...
            if result:
                found.append(result)
                print(f"  {GRN}[FOUND]{R}  {WHT}{result[0]:<40}{R}  {GRY}{result[1]}{R}")
            print(f"  {GRY}Scanning... {done}/{len(SUBDOMAINS)}{R}", end="\r")

    print(f"\n\n  {BLD}{WHT}{'─'*50}{R}")
    if found:
        print(f"  {GRN}{BLD}Total subdomain ditemukan: {len(found)}{R}")
    else:
        print(f"  {YLW}Tidak ada subdomain yang ditemukan.{R}")
    print(f"  {BLD}{WHT}{'─'*50}{R}\n")
    input(f"  {GRY}Tekan ENTER untuk kembali ke menu...{R}")
...
def main():
    while True:
        ...
        print(f"  {CYN}5.{R}  Port Scanner")
        print(f"  {CYN}6.{R}  DNS Lookup")
        print(f"  {CYN}7.{R}  Whois Domain")
        print(f"  {CYN}8.{R}  Subdomain Finder")
        print(f"  {CYN}9.{R}  Tentang tools ini")
        print(f"  {CYN}0.{R}  Keluar\n")
        pilihan = input(f"  {WHT}Pilih menu{R} {GRY}[0-9]{R}: ").strip()
